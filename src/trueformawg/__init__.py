...
class TrueFormAWG():
    ...
    def clear_ch_mem(self):
        self.device.write(f':SOURce{self.channel_num}:DATA:VOLatile:CLEar')
        print(f'Cleared memory of channel {self.channel_num}')
    ...
    def load_awf(self, AWFname, AWF):    
        ...
        bytes_sent = self.device.write_binary_values(f'SOUR{self.channel_num}:DATA:ARB {AWFname},', AWF, datatype='f', is_big_endian=True)
        print(bytes_sent)
    ...
    def select_awf(self, AWFname):
        ...
        self.device.write(f':SOUR{self.channel_num}:FUNC %s' % ('ARB'))
        # Select my arb from the internal memory of the instrument
        self.device.write(f':SOUR{self.channel_num}:FUNC:ARB "%s"' % (AWFname))
    
    def avalable_memory(self):
        print('Available memory (in samples): ', self.device.query(f'SOURce{self.channel_num}:DATA:VOLatile:FREE?'))
    ...
    def set_sample_rate(self, sampleRate):
        self.device.write(':SOURce%d:FUNCtion:ARBitrary:SRATe %G' % (self.channel_num, sampleRate))


    def set_amplitude(self, amplitude):
        self.device.write(':SOURce%d:VOLTage %G' % (self.channel_num, amplitude)) 
        
     
    def set_offset(self, offset):
        self.device.write(':SOURce%d:VOLTage:OFFSet %G' % (self.channel_num, offset))
        
        
    def set_Z_out_infinite(self):
        self.device.write(':OUTPut%d:LOAD %s' % (self.channel_num, 'INF'))

    def set_Z_out(self, z_out):
        self.device.write(':OUTPut%d:LOAD %s' % (self.channel_num, z_out))
    ...
    def combine_channels(self, source_channel = 'CH2'):
        ...
        if self.channel_num == 1:
            ...
            source_channel = 'CH1'
        self.device.write(':SOURce%d:COMBine:FEED %s' % (self.channel_num, source_channel))

    def set_indipendent(self):
        self.device.write(':SOURce%d:COMBine:FEED %s' % (self.channel_num, 'NONE'))

    def turn_on(self):
        self.device.write(':OUTPut%d %d' % (self.channel_num, 1)) 
        print('Awg turned on')
                        
    def turn_off(self):
        self.device.write(':OUTPut%d %d' % (self.channel_num, 0)) 
